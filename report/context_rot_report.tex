...
\begin{itemize}
  \item \textbf{Technical ambition.} The project goes beyond a standard RAG pipeline by implementing a bounded recursive controller over an external document environment and comparing it against three baselines under the same budget framework.
  \item \textbf{Engineering.} The codebase includes chunking and hybrid retrieval, resumable benchmark runners, trace export, budget and token accounting, retry/backoff guard rails for transient API failures, and a passing automated test suite of 53 tests.
  \item \textbf{Rigour.} The retained canonical run covers 2,003 evaluations, and the project also includes targeted follow-up audits such as the harder 500k challenging needle diagnostic and the matched-overlap audit for the Pro MuSiQue slice.
  \item \textbf{Research context.} The evaluation is grounded in prior work on long-context failure modes, recursive language models, RAG, LongBench, and MuSiQue \cite{liu2024lost, zhang2025rlm, lewis2020rag, bai2023longbench, trivedi2022musique}.
\end{itemize}
